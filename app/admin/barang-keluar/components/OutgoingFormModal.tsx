...
const OutgoingFormContent: React.FC<Omit<OutgoingFormModalProps, "isOpen">> = ({
  onClose,
  onSubmit,
  items,
  locations,
  ...
  const [availableStock, setAvailableStock] = useState<number>(() => items[0]?.stock || 0);
  ...
  const purposeOptions: DropdownOption[] = purposes.map((p) => ({
    value: p.name,
    label: p.name,
    icon: p.name.includes("Audit") ? Briefcase : p.name.includes("Servis") ? RefreshCw : ArrowUpFromLine,
  }));
...
export const OutgoingFormModal: React.FC<OutgoingFormModalProps> = ({
  ...
  return (
    <Modal
      ...
      description="Catat distribusi barang atau peminjaman logistik ke unit kerja Inspektorat."
      maxWidth="4xl"
    >
      {isOpen && (
        <OutgoingFormContent
